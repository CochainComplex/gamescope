...
- **E2 full-colour probe** (`GAMESCOPE_FRAMEGEN_RECORD_COLOR`) is an offline image-quality
  ...
  A/B/C triplets; `framegen_submit_planned` generates paired strengths 0/0.5/1 in one command
  buffer and copies them after `framegenTimestampEnd`; `scripts/framegen-color-eval.py` grades
  every candidate against the same B. The copy/write path is excluded from the algorithm GPU
  timestamp but synchronous host file I/O can visibly disturb the real-only capture cadence.
